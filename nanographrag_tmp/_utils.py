...
def is_valid_json(string: str) -> tuple[bool, str]:
    """
    判断字符串是否是有效的 JSON 格式
...
def check_and_fix_json(string: str) -> str:
    """
    检查并尝试修复简单的 JSON 格式错误
    ...
    is_valid, error_msg = is_valid_json(string)
    if is_valid:
        return string

    logger.info("Attempting to fix JSON: %s", string)
    REGEX_PATTERNS = [
        # (r':\s*"([^"]*)"([^"]*)"([^"]*)"', r': "\1\\"\2\\"\3"'),  # 转义值中的双引号
        (r',\s*([}\]])', r'\1'),  # 移除尾随逗号
        (r'\\([^"\\\/bfnrtu])', r'\\\\\1'),  # 修复无效的转义字符
        (r"'([^']+)':", r'"\1":'),  # 修复使用单引号的键名
        (r':\s*\'([^\']+)\'', r': "\1"'),  # 修复使用单引号的字符串值
        # (r'}\s*{', '},{'),  # 修复对象之间缺失的逗号
        # (r']\s*{', '],{'),  # 修复数组和对象之间缺失的逗号
        # (r'}\s*\[', '},\['),  # 修复对象和数组之间缺失的逗号
        # (r']\s*\[', '],\['),  # 修复数组之间缺失的逗号
        # (r'(["\d}\]]),\s*([{[])', r'\1,\2'),  # 修复元素之间缺失的逗号
        # (r'(["\w\d}\]]+)\s+([{[\w"])', r'\1,\2'),  # 修复任意元素之间缺失的逗号
        # (r'(["}])\s*([{\[])', r'\1,\2'),  # 修复对象/数组之间缺失的逗号
        # (r'(["}])\s*"', r'\1,"'),         # 修复字符串之间缺失的逗号
        # (r'"}(?!\s*[,}\]])', r'"},'),     # 修复对象属性之间缺失的逗号
    ]
    
    fixed_string = string
    for pattern, replacement in REGEX_PATTERNS:
        old_string = fixed_string
        fixed_string = re.sub(pattern, replacement, fixed_string)
        if old_string != fixed_string:
            logger.info("Applied fix %s -> %s", pattern, replacement)
            logger.info("Result: %s", fixed_string)
...
#         if json_str is None:
#             logger.error(f"Failed to locate JSON in response: {response}")
...
#         if not is_valid:
#             logger.warning(f"Invalid JSON, attempting to fix. JSON:{json_str} Error: {error_msg}")
#             fixed_json = check_and_fix_json(json_str)
#             logger.info("After fixing: %s", fixed_json)
        
#             return json.loads(fixed_json)
#         return json.loads(json_str)
#     except Exception as e:
#         logger.error(f"JSON conversion failed. Original text:\n{response}\n\nError: {str(e)}")
#         return 
